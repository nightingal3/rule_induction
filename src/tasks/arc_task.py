...
class ArcTask(BaseTask):
    ...
    def get_few_shot_examples(
        self, idx: int, add_example_input: bool = False, **kwargs
    ) -> str:
    ...
    def validate(self, idx: int, output_actions: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Error parsing output actions on {idx}. Returning False for now, validate manually."
                )
                return False
    ...
    def get_standard_prompt(self, idx: int) -> Tuple[str, str]:
        ...
        return (
            self.standard_prompt_wrap(few_shot_examples, input),
            self.test_examples_ids[idx],
        )
    ...
    def full_grammar_prompt_wrap(
        self, idx: int, few_shot_examples: str, input: str
    ) -> str:
        rule = self.rules[self.split][self.test_examples_ids[idx]]
        return prompt_with_true_grammar["user"].format(
            induced_grammar=rule, few_shot_examples=few_shot_examples, input=input
        )
    ...
    def grammar_induction_prompt_wrap(self, idx: int, few_shot_examples: str) -> str:
        ...
        return prompt_for_grammar_induction["user"].format(
            few_shot_examples=few_shot_examples, example_input=example_input
        )
