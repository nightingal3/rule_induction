...
class CherokeeTask(BaseTask):
    ...
    def __init__(
        ...
        tgt_lang: Literal["chr", "en"] = "en",
        use_dev: bool = False,
        use_curriculum: bool = False,
        **kwargs,
    ) -> None:
        self.train_data, self.test_data, self.dev_data = (
        ...
        self.vocab_df = (
            pd.read_csv(dictionary_file) if dictionary_file is not None else None
        )
        ...
        if self.vocab_df is not None:
            self.src_tgt_dict = dict(
                zip(self.vocab_df[self.src_lang], self.vocab_df[self.tgt_lang])
            )
            self.src_tgt_dict = {
                k: v for k, v in self.src_tgt_dict.items() if not pd.isna(v)
            }  # empty translations

        self.base_prompt = (
            base_prompt_chr_en if self.tgt_lang == "en" else base_prompt_en_chr
        )
        self.few_shot_examples_prompt = (
            few_shot_examples_chr_en_prompt
            if self.tgt_lang == "en"
            else few_shot_examples_en_chr_prompt
        )
        ...
        self.en_common_nouns = pickle.load(
            open("./data/cherokee/most_common_nouns.pkl", "rb")
        )
        self.en_common_verbs = pickle.load(
            open("./data/cherokee/most_common_verbs.pkl", "rb")
        )
        self.en_common_adjectives = pickle.load(
            open("./data/cherokee/most_common_adjs.pkl", "rb")
        )
        self.en_common_function_words = pickle.load(
            open("./data/cherokee/most_common_cconjs.pkl", "rb")
        )
    ...
    def retrieve_few_shot_examples_by_similarity(
        self, idx: int, num_examples: int = 5, metric: str = "cos_sim"
    ) -> str:
        if self.tgt_lang == "chr" and metric == "cos_sim":
            raise ValueError(
                "cos_sim is not supported for Cherokee, please use another metric."
            )
        ...
        if not return_ref_and_hyp_only:
            scores = {
                metric: self.eval_fns[metric].compute(
                    predictions=[output_actions], references=[reference]
                )
                for metric in self.eval_criteria
                if metric != "bertscore"
            }
            if "bertscore" in self.eval_criteria:
                scores["bertscore"] = self.eval_fns["bertscore"].compute(
                    predictions=[output_actions],
    ...
    def get_standard_prompt(
        self, idx: int
    ) -> Tuple[str, str]:  # TODO: refactor this signature later
        input = self.get_input(idx)
        few_shot_examples = self.get_few_shot_examples(idx)
        dict_prompt = None
        if self.src_tgt_dict is not None:
            matching_entries = utils.find_matching_dict_entries(
                input, self.src_tgt_dict
            )
            dict_prompt = self.dict_prompt_wrap(matching_entries)

        return (
            self.standard_prompt_wrap(
                few_shot_examples, input, dict_entries=dict_prompt
            ),
            "",
        )
    ...
    def get_special_prompt(
        ...
        if self.prompt_style == "full_grammar":
            ...
            if self.prompt_style == "grammar_induction":
                ...
                prompt_with_relevant_vocab = self.prompt_with_relevant_vocab_wrap(
                    vocab, self.get_input(idx)
                )
                return (
                    prompt_with_relevant_vocab,
                    completion["usage"]["completion_tokens"],
                    completion["usage"]["prompt_tokens"],
                )
    ...
    def get_examples_containing_vocab(
        self,
        vocab: str,
        upos: str,
        get_all_list: bool = False,
        example_selection: Literal["base", "shortest", "informative"] = "base",
        ...
        if get_all_list:
            return [
                (
                    self.train_data.iloc[ind][self.src_lang],
                    self.train_data.iloc[ind][self.tgt_lang],
                )
                for ind in inds
            ]
        ...
        few_shot_examples = self.few_shot_examples_wrap(
            subset[self.src_lang].tolist(), subset[self.tgt_lang].tolist()
        )
        prompt = prompt_for_vocab_induction["user"].format(
            few_shot_examples=few_shot_examples, word=vocab
        )
    ...
    def standard_prompt_wrap(
        self, few_shot_examples: str, input: str, dict_entries: Optional[str] = None
    ) -> str:
        if dict_entries is None:
            return self.base_prompt["user"].format(
                few_shot_examples=few_shot_examples, input=input
            )
        else:
            return self.base_prompt["user_dict"].format(
    ...
    def prompt_with_induced_grammar_wrap(
        self, induced_grammar: str, few_shot_examples: str, input: str
    ) -> str:
    ...
    def dict_prompt_wrap(self, matching_entries: dict) -> str:
        matching_entries_str = "\n".join(
            [f"{k} -> {v}" for k, v in matching_entries.items()]
        )
        return prompt_for_dict_lookup.format(matching_entries=matching_entries_str)

    def few_shot_examples_wrap(
        self, few_shot_inputs: List, few_shot_outputs: List
    ) -> str:
        examples = ""
