...
class NacloTask(BaseTask):
    ...
    def __init__(
        self,
        prompt_style: Literal["base", "grammar_induction"],
        ...
        **kwargs,
        ...
        for filename in os.listdir(
            "./data/naclo-puzzlingmachines/data_public_reference_data_dev"
        ):
            if filename.endswith(".json"):
                with open(
                    f"./data/naclo-puzzlingmachines/data_public_reference_data_dev/{filename}"
                ) as f:
                    ...
                    for test_problem in dev_problem["test"]:
                        test_problem_data = {
                            "test": {
                                "source_language_test": dev_problem["source_language"]
                                if test_problem[2] == ">"
                                else dev_problem["target_language"],
                                "target_language_test": dev_problem["target_language"]
                                if test_problem[2] == ">"
                                else dev_problem["source_language"],
                                "input_test": test_problem[0]
                                if test_problem[2] == ">"
                                else test_problem[1],
                                "reference_test": test_problem[1]
                                if test_problem[2] == ">"
                                else test_problem[0],
                            }
                        }
                        self.dev_problems.append(
                            {**background_info, **test_problem_data}
                        )

        self.test_problems = []
        for filename in os.listdir(
            "./data/naclo-puzzlingmachines/data_public_data_test"
        ):
            if filename.endswith(".json"):
                with open(
                    f"./data/naclo-puzzlingmachines/data_public_data_test/{filename}"
                ) as f:
                    ...
                    for curr_test_problem in test_problem["test"]:
                        test_problem_data = {
                            "test": {
                                "source_language_test": test_problem["source_language"]
                                if curr_test_problem[2] == ">"
                                else test_problem["target_language"],
                                "target_language_test": test_problem["target_language"]
                                if curr_test_problem[2] == ">"
                                else test_problem["source_language"],
                                "input_test": curr_test_problem[0]
                                if curr_test_problem[2] == ">"
                                else curr_test_problem[1],
                                "reference_test": curr_test_problem[1]
                                if curr_test_problem[2] == ">"
                                else curr_test_problem[0],
                            }
                        }
        ...
        self.eval_fns = {metric: evaluate.load(metric) for metric in self.eval_criteria}
    ...
    def tfidf_vectorize(self) -> None:
        ...
        for i in range(len(self.test_data)):
            ...
            src_ind = (
                eng_ind
                if self.test_data[i]["test"]["source_language_test"] == "english"
                else 1 - eng_ind
            )
            self.tfidf[i] = vectorizer.fit_transform(
                [demonstration[src_ind] for demonstration in self.test_data[i]["train"]]
            )
            # test
    ...
    def process_en_svo(self) -> None:
        ...
        for idx in range(len(self.test_data)):
            ...
            for d in demonstrations:
                ...
                en_lemma_sent = " ".join(
                    [word.lemma for word in self.nlp(en_sent).sentences[0].words]
                )
                ...
                for sent in doc.sentences:
                    ...
                    self.sentence_store[idx].add_sentence(
                        (en_sent, en_lemma_sent, other_sent),
                        categories,
                        unique_nouns,
                        unique_verbs,
                        unique_adjs,
    ...
    def guess_word_order(self, idx: int) -> None:
        ...
        message = [
            {
                "role": "system",
                "content": "Here's a translation puzzle. Try to figure out the word order for sentences in the non-English language.",
            },
    ...
    def guess_grammar_rules(self, idx: int) -> None:
        ...
        for contrast_set in contrast_sets:
            ...
            prompt = grammar_prompt.format(
                set1_name=set1_name,
                set2_name=set2_name,
                demonstrations1=set1_demonstrations,
                demonstrations2=set2_demonstrations,
            )
            message = [
                {
                    "role": "system",
                    "content": "Here's a translation puzzle. Here are two sets of sentences representing the difference between {set1_name} and {set2_name} in another language. Your job is to deduce the rule that underlies this difference, or say 'no difference' if you believe no difference exists.",
                },
                {"role": "user", "content": prompt},
            ]
            print("PROMPT: ", prompt)
    ...
    def find_similar_words(self, idx: int, test_word: str, src_lang: str) -> List[str]:
        ...
        for demonstration in self.test_data[idx]["train"]:
            ...
            docs_with_most_similar_words.append(
                (
                    sentence_min_distance,
                    demonstration[src_ind],
                    demonstration[1 - src_ind],
                )
            )

        sorted_docs_with_most_similar_words = sorted(
            docs_with_most_similar_words, key=lambda x: x[0]
        )
    ...
    def get_vocab_induction_prompt(self, idx: int, threshold: int = 4) -> List[str]:
        def generator() -> str:
            ...
            while test_index < len(test_words):
                ...
                demonstrations_by_relevance = self.find_similar_words(
                    idx, word, src_lang=test_src_lang
                )
                relevant_demonstrations = [
                    (demonstration[1], demonstration[2])
                    for demonstration in demonstrations_by_relevance
                    if demonstration[0] <= threshold
                ]
                demonstrations_str = ""
                ...
                yield vocab_induction_selected_examples_prompt.format(
                    src_lang=test_src_lang,
                    tgt_lang=test_tgt_lang,
                    input=word,
                    demonstrations=demonstrations_str,
                    word_order=ground_truth_lang_features[test_src_lang],
                    meta=meta,
                )
                test_index += 1
    ...
    def get_meta(self, idx: int) -> dict:
        return {
            "source_language": self.test_data[idx]["source_language"].strip(),
            "target_language": self.test_data[idx]["target_language"].strip(),
            "meta": self.test_data[idx]["meta"],
        }
    ...
    def get_few_shot_examples(self, idx: int) -> str:
        ...
        for demonstration in demonstrations:
            examples_prompt += few_shot_examples_src_tgt_prompt.format(
                src_lang=metadata["source_language"],
                tgt_lang=metadata["target_language"],
                input=demonstration[0],
    ...
    def validate(
        self, idx: int, output_text: str, return_ref_and_hyp_only: bool = False
    ) -> dict:
        ...
        if not return_ref_and_hyp_only:
            scores = {
                metric: self.eval_fns[metric].compute(
                    predictions=[output_text], references=[answer]
                )
                for metric in self.eval_criteria
                if metric != "bertscore"
    ...
    def validate_all(self, refs: List, hyps: List) -> Dict[str, List]:
        scores = {
            metric: self.eval_fns[metric].compute(predictions=hyps, references=refs)
            for metric in self.eval_criteria
            if metric != "bertscore"
        }
        return scores
    ...
    def get_standard_prompt(self, idx: int) -> Tuple[str, str]:
        ...
        return (
            self.standard_prompt_wrap(
                src_lang, tgt_lang, meta_str, few_shot_examples, input
            ),
            f"{src_lang}-{tgt_lang}",
        )
    ...
    def get_special_prompt(
        ...
        if self.prompt_style == "full_grammar":
            ...
            if self.prompt_style == "grammar_induction":
                ...
                if use_cached:
                    ...
                    if "gpt" in backend:
                        ...
                        while self.grammar_induction_loop and not converged:
                            ...
                            induced_grammar_new = completion["choices"][0]["message"][
                                "content"
                            ]
                ...
                meta = self.get_meta(idx)["meta"]
                prompt_with_induced_vocab = vocab_induction_final_prompt.format(
                    rules="\n".join(induced_vocab),
                    tgt_lang=self.test_data[idx]["test"]["target_language_test"],
                    src_lang=self.test_data[idx]["test"]["source_language_test"],
                    input=self.get_input(idx),
    ...
    def standard_prompt_wrap(
        src_lang: str, tgt_lang: str, meta: str, few_shot_examples: str, input: str
    ) -> str:
