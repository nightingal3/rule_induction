...
def make_completion_fn(model_name: str) -> Callable:
    ...
    assert len(model_addr) > 0, f"Model {client_name} not found"
    ...
    def get_completion_open_llm(
        ...
        **kwargs,
        ...
        if isinstance(prompt, list):
            ...
            prompt_text = prompt
        output = client.generate(prompt_text, temperature=temp, max_new_tokens=512)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Prompt open source (huggingface models) with task specs"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument("--dataset", required=True, choices=["scan", "cogs", "colours", "functions"])
    parser.add_argument(
        "--split",
        default="simple",
        choices=[
